    ...
    output$rendered <-   renderUI({
      list(
        checkboxGroupInput("qualdim",  tags$span("Auswahl der Qualitätsdimension",   
                           ...
                           choices = cho,
                           selected = c("ala")),
        # makeCheckboxTooltip(checkboxValue = "ma", buttonLabel = "?", Tooltip = "Look! I can produce a tooltip!")
        makeCheckboxTooltip(checkboxValue=cho,buttonLabel=rep("?",length(cho)),Tooltip=rep("Look",length(cho)))
      )
    })
